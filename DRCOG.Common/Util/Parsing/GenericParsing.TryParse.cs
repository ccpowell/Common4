...
namespace DRCOG.Common.Util
{
	...
	{
		private static readonly Dictionary<Type, MethodInfo> _tryParseMethods = new Dictionary<Type, MethodInfo>();
		...
		{
			if (!_tryParseMethods.ContainsKey(type))
			{
				lock (((ICollection)_tryParseMethods).SyncRoot)
				{
					if (!_tryParseMethods.ContainsKey(type))
					{
						_tryParseMethods.Add(type, FindTryParseMethod(type));
					}
				}
			}

			return _tryParseMethods[type];
		}
